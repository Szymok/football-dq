...
impl UnderstatExtractor {
    ...
    /// Terminarz gier wg sezonu i ligi (zawarty w tagach <script> w treści HTML na landing page'u)
    pub async fn read_schedule(&self, _include_matches_without_data: bool, force_cache: bool) -> Result<Vec<String>> {
    ...
    pub async fn read_team_match_stats(&self, force_cache: bool) -> Result<Vec<String>> {
        ...
        for season in &self.config.seasons {
            for league in &self.config.leagues {
                let url = format!("{}/league/{}/{}", self.base_url, league, season);
                let cache_file = self.config.data_dir.join(format!("team_match_stats_{}_{}.html", season, league));
